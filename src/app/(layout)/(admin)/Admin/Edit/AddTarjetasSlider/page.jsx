...
export default function Home() {
    ...
    const { user, introVideo, userDB, setUserProfile, option, succes, setUserSuccess, success,  cliente, setCliente, cart, setCart, modal, setModal } = useUser()
    ...
    function saveTarjetas(e) {
        ...
        writeUserData(`/Cliente/${option}`, { [key]: data3 }, setUserSuccess)
    }
    console.log(data3)
    useEffect(() => {
                ...
                <div className="relative w-[95%] lg:w-[50%] bg-white border-b rounded-[10px] pt-16 pb-16 lg:pb-4 px-5">
                    ...
                    <form className="relative  pt-5 sm:col-span-3 mb-5 pb-5 border-b-[.5px] " onSubmit={saveTarjetas} >
                        <div className="sm:col-span-3 mb-5 pb-5 border-b-[.5px] border-[#666666]">
                            ...
                            <label htmlFor="first-name" className="block text-[12px] font-medium leading-6 text-gray-900">URL de empresa</label>
                            <input type="text" name={`companyURL`} onChange={(e) => onChangeHandler3(e)} className="block w-full rounded-md border-0 p-1.5 mt-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-[12px] sm:leading-6" defaultValue={data3[`companyURL`]} />
                            <br />
